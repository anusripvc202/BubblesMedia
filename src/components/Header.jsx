...
export default function Header({ cartCount, wishlistCount, onCartOpen, onQuoteOpen, onSearch }) {
  ...
  return (
    <header style={{ width: '100%', background: 'var(--bg-white)', borderBottom: '1px solid var(--border-color)', zIndex: 100, position: 'sticky', top: 0 }}>
      ...
      <div className="hide-on-mobile" style={{ background: 'var(--primary)', color: 'white', padding: '4px 0', fontSize: '0.72rem', fontWeight: '600' }}>
      ...
      {/* Main Header Bar - single row: Logo | Categories | Search | Controls */}
      <div className="container-layout header-main-bar" style={{ padding: '8px 24px', display: 'flex', alignItems: 'center', gap: '12px' }}>
        ...
        <button className="hide-on-mobile header-categories-btn" style={{
          ...
          All Categories
          <svg width="9" height="9" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3.5" strokeLinecap="round" strokeLinejoin="round" style={{ transform: 'rotate(90deg)' }}>
            <path d="m9 18 6-6-6-6"></path>
          </svg>
        </button>

        {/* Search Bar - grows to fill remaining space */}
        <form onSubmit={handleSearchSubmit} className="header-search-form" style={{ flex: 1, minWidth: '160px', display: 'flex', background: 'white', borderRadius: '6px', overflow: 'hidden', border: '1px solid var(--border-color)' }}>
          <input
            type="text"
          ...
          />
          <button type="submit" style={{ background: 'var(--primary)', color: 'white', padding: '0 16px', display: 'flex', alignItems: 'center', gap: '5px', fontWeight: '700', fontSize: '0.83rem', flexShrink: 0 }}>
        ...
        {/* Right Controls */}
        <div className="header-right-controls" style={{ display: 'flex', alignItems: 'center', gap: '12px', flexShrink: 0 }}>
          {/* Call Us Now */}
          <div className="hide-on-mobile" style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <div style={{ background: 'var(--primary-light)', color: 'var(--primary)', width: '34px', height: '34px', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              📞
            </div>
            <div>
              <span style={{ fontSize: '0.62rem', color: 'var(--text-light)', display: 'block' }}>Call Us Now</span>
              <span style={{ fontSize: '0.82rem', fontWeight: '700', color: 'var(--secondary)' }}>+91 93815 66798</span>
            </div>
          </div>

          {/* Account */}
          <div className="hide-on-mobile" style={{ display: 'flex', alignItems: 'center', gap: '8px', borderLeft: '1px solid var(--border-color)', paddingLeft: '12px' }}>
            <div style={{ background: 'var(--primary-light)', color: 'var(--primary)', width: '34px', height: '34px', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              👤
            </div>
            <div>
              <span style={{ fontSize: '0.62rem', color: 'var(--text-light)', display: 'block' }}>My Account</span>
              <span style={{ fontSize: '0.82rem', fontWeight: '700', color: 'var(--secondary)', cursor: 'pointer' }} onClick={onQuoteOpen}>
                Sign In / Register
              </span>
            </div>
          </div>
          ...
          {/* Wishlist */}
          <button className="hide-on-mobile" style={{ position: 'relative', color: 'var(--secondary)', padding: '4px' }}>
