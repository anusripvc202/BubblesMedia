...
export default function SolutionsGrid({ searchTerm, activeCategory, onAddToCart, onEnquire }) {
    ...
    <section id="solutions" style={{ marginBottom: '40px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
        <div>
          ...
          <p style={{ fontSize: '0.8rem', color: 'var(--text-light)', marginTop: '2px' }}>High-converting digital services ready to scale your business</p>
        </div>
      ...
      {filteredSolutions.length === 0 ? (
        <div style={{
          ...
          <p style={{ fontWeight: '600', color: 'var(--text-dark)' }}>No Solutions Found</p>
          <p style={{ fontSize: '0.85rem', marginTop: '4px' }}>We couldn't find matches for "{searchTerm}". Try another search keyword.</p>
        </div>
      ) : (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '24px' }}>
          {filteredSolutions.map((sol) => (
            ...
            >
              ...
              {/* Title */}
              <h3 style={{ fontSize: '1.1rem', fontWeight: '800', marginBottom: '4px', color: 'var(--text-dark)' }}>
                {sol.title}
              </h3>

              {/* Pricing */}
              <div style={{ display: 'flex', alignItems: 'baseline', gap: '4px', marginBottom: '16px' }}>
                <span style={{ fontSize: '0.8rem', color: 'var(--text-light)', fontWeight: '500' }}>Starting from</span>
                <span style={{ fontSize: '1.3rem', fontWeight: '800', color: 'var(--primary)', fontFamily: 'var(--font-display)' }}>{sol.price}</span>
                {sol.period && <span style={{ fontSize: '0.8rem', color: 'var(--text-light)', fontWeight: '500' }}>{sol.period}</span>}
              </div>
